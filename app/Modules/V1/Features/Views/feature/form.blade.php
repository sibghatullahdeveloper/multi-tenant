        ...
        <div class="row">
            <div class="col-12">
              <div class="card card-info">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{route('features.add.form',['role' => \Auth::user()->role->slug])}}">
                  @csrf
                  <div class="card-body">
                      ...
                      <div class="col-sm-10">
                        <select class="form-control select2" name="display_type" style="width: 100%;">
                          ...
                          @foreach ($input_field_types as $input_field_type)
                            <option value="{{old('display_type',$input_field_type)}}" @if (old('display_type',$input_field_type) == $input_field_type) selected="selected" @endif>{{$input_field_type}}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                      ...
                      <div class="col-sm-10">
                        <select class="form-control select2"  multiple="multiple" name="category[]" style="width: 100%;">
                          ...
                          @foreach ($categories as $category)
                            <option value="{{$category->id}}" @if (old('category.'.$loop->index,'') == $category->id) selected="selected" @endif>{{$category->title}}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                    <div class="form-group row">
                      ...
                      <div class="col-sm-10">
                        <input type="number" class="form-control" min="1" name="sort_order"  id="sort_order" placeholder="Sort Order" value="{!! old('sort_order','') !!}">
                      </div>
                    </div>
                    <div class="form-group row">
                  ...
                  <div class="card-footer">
                    <button type="submit" class="btn btn-info">Add Feature</button>
